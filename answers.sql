...
SELECT m.name AS model_name, b.name AS brand_name, b.headquarters AS headquarters FROM models AS m JOIN brands AS b USING (brand_id) WHERE b.name = 'Ford' and m.name = 'Mustang'; 	
...
SELECT COUNT(name) AS count FROM models WHERE brand_id = 'for';
...
SELECT m.year AS year, m.name AS name, m.model_id AS model_id, a.name AS name FROM models AS m LEFT JOIN awards AS a ON m.model_id = a.winner_id WHERE m.year >= 1964;
...
SELECT name, founded FROM brands WHERE brand_id NOT IN (SELECT brand_id FROM models);
...
SELECT brand_id FROM models GROUP BY brand_id HAVING COUNT(name) > 5; 
